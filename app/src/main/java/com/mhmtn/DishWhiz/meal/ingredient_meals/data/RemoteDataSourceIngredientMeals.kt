package com.mhmtn.DishWhiz.meal.ingredient_meals.data
...
class RemoteDataSourceIngredientMeals(private val api: API) : MealDataSource {
    override suspend fun getMeals(name: String): Resource<Meals> {
        val response = try {
            api.getIngredientMeals(name)
        } catch (e: Exception) {
